...
target_dir="${CARGO_TARGET_DIR:-/var/tmp/rch_target_franken_engine_seqlock_rollout_guard_${timestamp}}"
...
run_mode() {
  case "$mode" in
    check)
      run_step "cargo check -p frankenengine-engine --test seqlock_rollout_guard --bin franken_seqlock_rollout_guard" \
        cargo check -p frankenengine-engine --test seqlock_rollout_guard --bin franken_seqlock_rollout_guard
      ;;
    test)
      run_step "cargo test -p frankenengine-engine --test seqlock_rollout_guard --bin franken_seqlock_rollout_guard" \
        cargo test -p frankenengine-engine --test seqlock_rollout_guard --bin franken_seqlock_rollout_guard
      ;;
    clippy)
      run_step "cargo clippy -p frankenengine-engine --test seqlock_rollout_guard --bin franken_seqlock_rollout_guard -- -D warnings" \
        cargo clippy -p frankenengine-engine --test seqlock_rollout_guard --bin franken_seqlock_rollout_guard -- -D warnings
      ;;
    run)
