...
if ! command -v rch >/dev/null 2>&1; then
  echo "rch is required for shape transition lattice heavy commands" >&2
  exit 2
fi
...

  {
    while IFS= read -r line || [[ -n "${line}" ]]; do
      ...
      if [[ "${line}" == *"Remote toolchain failure, falling back to local"* ||
        ...
        pkill -f 'franken_shape_lattice_bundle' 2>/dev/null || true
        pkill -f 'shape_transition_lattice_cli' 2>/dev/null || true
...
emit_run_artifact_paths() {
  ...
  local manifest_path="${artifact_dir}/run_manifest.json"
  local shape_manifest_path="${artifact_dir}/shape_lattice_manifest.json"
  ...
  for path in \
    ...
    "${trace_ids_path}"; do
    if [[ ! -f "${path}" ]]; then
      echo "shape transition lattice bundle is incomplete; missing ${path}" >&2
      return 1
    fi
  done

  echo "shape transition lattice run manifest: ${manifest_path}"
  echo "shape transition lattice manifest: ${shape_manifest_path}"
  echo "shape transition lattice events: ${events_path}"
  echo "shape transition lattice commands: ${commands_path}"
  echo "shape transition lattice trace ids: ${trace_ids_path}"
}

case "${mode}" in
  check)
    run_step \
      "cargo check -p frankenengine-engine --bin franken_shape_lattice_bundle --test shape_transition_lattice_cli" \
      cargo check -p frankenengine-engine --bin franken_shape_lattice_bundle --test shape_transition_lattice_cli
    ;;
  test)
    run_step \
      "cargo test -p frankenengine-engine --test shape_transition_lattice_cli" \
      cargo test -p frankenengine-engine --test shape_transition_lattice_cli
    ;;
  clippy)
    run_step \
      "cargo clippy -p frankenengine-engine --bin franken_shape_lattice_bundle --test shape_transition_lattice_cli -- -D warnings" \
      cargo clippy -p frankenengine-engine --bin franken_shape_lattice_bundle --test shape_transition_lattice_cli -- -D warnings
    ;;
  run)
    run_step \
      "cargo run -p frankenengine-engine --bin franken_shape_lattice_bundle -- --out-dir ${run_dir}" \
      cargo run -p frankenengine-engine --bin franken_shape_lattice_bundle -- --out-dir "${run_dir}"
