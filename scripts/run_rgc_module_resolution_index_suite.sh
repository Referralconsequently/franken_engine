...
artifact_root_input="${RGC_MODULE_RESOLUTION_INDEX_ARTIFACT_ROOT:-artifacts/rgc_module_resolution_index}"
rch_timeout_seconds="${RCH_EXEC_TIMEOUT_SECONDS:-900}"
cargo_build_jobs="${CARGO_BUILD_JOBS:-1}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
default_target_dir="${root_dir}/target_rch_module_resolution_index"
...
if ! command -v rch >/dev/null 2>&1; then
  echo "rch is required for RGC module resolution index heavy commands" >&2
  exit 2
fi
...
emit_bridge_artifacts() {
  ...
  marker_prefix="__RGC_MODULE_INDEX_ARTIFACT__"

  run_step \
    "RGC_MODULE_RESOLUTION_INDEX_ARTIFACT_DIR=${run_dir} cargo test -p frankenengine-engine --test module_resolution_index module_resolution_index_artifact_bridge_emits_bundle_when_env_is_set -- --exact --nocapture && stream artifact bundle" \
    env \
    "RGC_MODULE_RESOLUTION_INDEX_ARTIFACT_DIR=${run_dir}" \
    "RGC_MODULE_RESOLUTION_INDEX_COMMANDS_JSON=${commands_json}" \
    sh -lc \
    'set -eu; cargo test -p frankenengine-engine --test module_resolution_index module_resolution_index_artifact_bridge_emits_bundle_when_env_is_set -- --exact --nocapture; for artifact_name in "$@"; do printf "%s:BEGIN:%s\n" "__RGC_MODULE_INDEX_ARTIFACT__" "$artifact_name"; cat "${RGC_MODULE_RESOLUTION_INDEX_ARTIFACT_DIR}/${artifact_name}"; printf "\n%s:END:%s\n" "__RGC_MODULE_INDEX_ARTIFACT__" "$artifact_name"; done' \
    sh \
    "${required_artifacts[@]}"
...
run_mode() {
  ...
  case "$mode" in
    check)
      run_step \
        "cargo check -p frankenengine-engine --test module_resolution_index" \
        cargo check -p frankenengine-engine --test module_resolution_index
      ;;
    test)
      run_step \
...
echo "rgc module resolution index artifacts: ${run_dir}"
