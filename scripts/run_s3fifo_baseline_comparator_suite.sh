...
artifact_root="${S3FIFO_BASELINE_COMPARATOR_ARTIFACT_ROOT:-artifacts/s3fifo_baseline_comparator}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
target_dir="${CARGO_TARGET_DIR:-/data/projects/franken_engine/target_rch_s3fifo_baseline_comparator}"
...
trace_id="${S3FIFO_BASELINE_TRACE_ID:-trace.rgc.620a}"
decision_id="${S3FIFO_BASELINE_DECISION_ID:-decision.rgc.620a}"
policy_id="${S3FIFO_BASELINE_POLICY_ID:-policy.rgc.620a}"
run_id="run-s3fifo-baseline-comparator-${timestamp}"
source_commit="$(git rev-parse HEAD 2>/dev/null || echo unknown)"
suite_commands_path="${run_dir}/suite_commands.txt"
step_logs_dir="${run_dir}/step_logs"

contract_doc="docs/RGC_S3FIFO_BASELINE_COMPARATOR_V1.md"
contract_json="docs/rgc_s3fifo_baseline_comparator_v1.json"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required for S3-FIFO baseline comparator artifacts" >&2
  exit 2
fi

if ! command -v rch >/dev/null 2>&1; then
  echo "rch is required for S3-FIFO baseline comparator heavy commands" >&2
  exit 2
fi
...
verify_bundle() {
  ...
  jq -e '.schema_version == "franken-engine.s3fifo-baseline-comparator.run-manifest.v1"' \
    "${run_dir}/run_manifest.json" >/dev/null
  jq -e '.schema_version == "franken-engine.cache-trace-corpus.v1" and (.cases | length == 5)' \
    "${run_dir}/cache_trace_corpus_manifest.json" >/dev/null
  jq -e '.baseline_policy_name == "single_queue_fifo" and .candidate_policy_name == "s3_fifo"' \
    "${run_dir}/cache_policy_baseline_report.json" >/dev/null
  jq -e '.schema_version == "franken-engine.s3fifo-adoption-wedge.v1"' \
    "${run_dir}/s3fifo_adoption_wedge_contract.json" >/dev/null
}

run_mode() {
  case "$mode" in
    check)
      run_step "cargo check -p frankenengine-engine --lib --test module_cache_integration --bin franken_s3fifo_baseline_comparator" \
        cargo check -p frankenengine-engine --lib --test module_cache_integration --bin franken_s3fifo_baseline_comparator
      ;;
    test)
      run_step "cargo test -p frankenengine-engine --lib module_cache::tests:: -- --nocapture" \
        cargo test -p frankenengine-engine --lib module_cache::tests:: -- --nocapture
      run_step "cargo test -p frankenengine-engine --test module_cache_integration -- --nocapture" \
        cargo test -p frankenengine-engine --test module_cache_integration -- --nocapture
      ;;
    clippy)
      run_step "cargo clippy -p frankenengine-engine --lib --test module_cache_integration --bin franken_s3fifo_baseline_comparator -- -D warnings" \
        cargo clippy -p frankenengine-engine --lib --test module_cache_integration --bin franken_s3fifo_baseline_comparator -- -D warnings
      ;;
    run)
      run_step "cargo run -p frankenengine-engine --bin franken_s3fifo_baseline_comparator -- --artifact-dir ${run_dir} --trace-id ${trace_id} --decision-id ${decision_id} --policy-id ${policy_id} --run-id ${run_id} --generated-at-utc ${generated_at_utc} --source-commit ${source_commit} --toolchain ${toolchain} --summary" \
        cargo run -p frankenengine-engine --bin franken_s3fifo_baseline_comparator -- \
...
write_manifest() {
  ...
  {
    echo "{"
    echo '  "schema_version": "franken-engine.s3fifo-baseline-comparator-suite.v1",'
    echo '  "bead_id": "bd-1lsy.7.20.1",'
    echo '  "component": "rgc_s3fifo_baseline_comparator_suite",'
    echo "  \"mode\": \"${mode}\","
    ...
    echo "    \"corpus_manifest\": \"${run_dir}/cache_trace_corpus_manifest.json\","
    echo "    \"baseline_report\": \"${run_dir}/cache_policy_baseline_report.json\","
    echo "    \"adoption_wedge\": \"${run_dir}/s3fifo_adoption_wedge_contract.json\","
    ...
    echo "    \"cat ${run_dir}/cache_trace_corpus_manifest.json\","
    echo "    \"cat ${run_dir}/cache_policy_baseline_report.json\","
