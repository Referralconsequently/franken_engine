use frankenengine_engine::module_cache::{
    S3FifoBaselineArtifactContext, emit_default_s3fifo_baseline_bundle,
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let bundle = emit_default_s3fifo_baseline_bundle(&context)
        .map_err(|error| format!("failed to write S3-FIFO baseline bundle: {error}"))?;

    if summary {
        println!("{}", render_s3fifo_baseline_summary(&bundle.report));
    } else {
        println!(
            "{}",
            serde_json::to_string_pretty(&serde_json::json!({
                "artifact_dir": bundle.artifact_dir,
                "run_manifest_path": bundle.run_manifest_path,
                "trace_ids_path": bundle.trace_ids_path,
                "written_files": bundle.written_files,
                "case_count": bundle.report.cases.len(),
                "baseline_policy_name": bundle.report.baseline_policy_name,
                "candidate_policy_name": bundle.report.candidate_policy_name,
            }))
...
fn build_command_line(context: &S3FifoBaselineArtifactContext) -> String {
    format!(
        "cargo run -p frankenengine-engine --bin franken_s3fifo_baseline_comparator -- --artifact-dir {} --trace-id {} --decision-id {} --policy-id {} --run-id {} --generated-at-utc {} --source-commit {} --toolchain {}",
...
fn usage() -> String {
    [
        "franken_s3fifo_baseline_comparator usage:",
