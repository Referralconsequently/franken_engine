...
use frankenengine_engine::law_mining::{ArtifactContext, emit_default_law_mining_bundle};
...
fn run(args: Vec<String>) -> Result<(), String> {
    ...
    let bundle = emit_default_law_mining_bundle(&context)
        .map_err(|error| format!("failed to write law-mining bundle: {error}"))?;

    if summary {
        let summary_md = std::fs::read_to_string(&bundle.summary_path)
            .map_err(|error| format!("failed to read summary: {error}"))?;
        println!("{summary_md}");
    } else {
        println!(
            "{}",
...
fn build_command_line(context: &ArtifactContext) -> String {
    format!(
        "cargo run -p frankenengine-engine --bin franken_law_mining -- --artifact-dir {} --trace-id {} --decision-id {} --policy-id {} --run-id {} --generated-at-utc {} --source-commit {} --toolchain {}",
