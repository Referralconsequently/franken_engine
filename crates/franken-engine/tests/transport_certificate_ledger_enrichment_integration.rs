...
fn enrichment_transport_error_display_no_duplicates_between_variants() {
    ...
    let strs = [e1.to_string(), e2.to_string(), e3.to_string(), e4.to_string()];
...
fn enrichment_classify_outcome_boundary_full_transport() {
    ...
    assert_eq!(classify_outcome(949_999), TransportOutcome::PartialTransport);
}

#[test]
fn enrichment_classify_outcome_boundary_partial_transport() {
    assert_eq!(classify_outcome(700_000), TransportOutcome::PartialTransport);
...
fn enrichment_evaluate_same_cell_full_transport_all_artifact_kinds() {
    let cell = cell_x86_zen4();
    for kind in ArtifactKind::ALL {
        let cert = eval(*kind, &format!("same-{}", kind.as_str()), &cell, &cell, 1_000_000, 1_000_000);
...
fn enrichment_evaluate_cross_arch_arch_sensitive_always_incompatible() {
    ...
    for kind in &sensitive_kinds {
        let cert = eval(*kind, &format!("xarch-{}", kind.as_str()), &src, &tgt, 1_000_000, 800_000);
        assert_eq!(
            cert.outcome,
            TransportOutcome::Incompatible,
...
fn enrichment_evaluate_cross_arch_non_sensitive_uses_residual() {
    ...
    for kind in &non_sensitive_kinds {
        let cert = eval(*kind, &format!("xarch-ns-{}", kind.as_str()), &src, &tgt, 1_000_000, 800_000);
...
fn enrichment_evaluate_certificate_id_deterministic_same_inputs() {
    ...
    let h = hash("det-test");
    let c1 = evaluate_transport(ArtifactKind::CacheEntry, h, &src, &tgt, 1_000_000, 900_000).unwrap();
    let c2 = evaluate_transport(ArtifactKind::CacheEntry, h, &src, &tgt, 1_000_000, 900_000).unwrap();
...
fn enrichment_evaluate_certificate_id_differs_for_different_artifacts() {
    let src = cell_x86_zen4();
    let tgt = cell_x86_alder();
    let c1 = eval(ArtifactKind::CacheEntry, "art-a", &src, &tgt, 1_000_000, 900_000);
    let c2 = eval(ArtifactKind::CacheEntry, "art-b", &src, &tgt, 1_000_000, 900_000);
    assert_ne!(c1.certificate_id, c2.certificate_id);
}

#[test]
fn enrichment_evaluate_performance_loss_complement() {
    let cert = eval(ArtifactKind::ProfileData, "loss", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 750_000);
    assert_eq!(cert.residual_fraction_millionths + cert.performance_loss_millionths(), 1_000_000);
}

#[test]
fn enrichment_evaluate_same_hardware_check() {
    let cell = cell_x86_zen4();
    let cert = eval(ArtifactKind::RewriteRule, "hw", &cell, &cell, 1_000_000, 1_000_000);
...
fn enrichment_evaluate_different_arch_same_hardware_false() {
    let cert = eval(ArtifactKind::ProfileData, "xarch", &cell_x86_zen4(), &cell_arm_nv2(), 1_000_000, 500_000);
    assert!(!cert.same_arch_family());
    assert!(!cert.same_hardware());
}
...
fn enrichment_evaluate_certificate_serde_roundtrip() {
    let cert = eval(ArtifactKind::SynthesizedKernel, "serde-cert", &cell_x86_avx512(), &cell_x86_zen4(), 1_000_000, 700_000);
...
fn enrichment_evaluate_certificate_display_contains_key_info() {
    let cert = eval(ArtifactKind::AotModule, "disp", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 800_000);
...
fn enrichment_build_ledger_basic_then_validate() {
    let cert = eval(ArtifactKind::CacheEntry, "led-basic", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 800_000);
    let components = vec![
...
fn enrichment_build_ledger_empty_components_valid() {
    let cert = eval(ArtifactKind::RewriteRule, "led-empty", &cell_x86_zen4(), &cell_x86_zen4(), 1_000_000, 1_000_000);
...
fn enrichment_build_ledger_exceeds_cert_source_returns_error() {
    let cert = eval(ArtifactKind::RewriteRule, "exceed", &cell_x86_zen4(), &cell_x86_zen4(), 500_000, 500_000);
...
fn enrichment_build_ledger_id_prefixed_with_ledger() {
    let cert = eval(ArtifactKind::ProfileData, "id-test", &cell_arm_nv2(), &cell_arm_a78(), 1_000_000, 900_000);
    let ledger = build_residual_ledger(&cert, vec![]).unwrap();
...
fn enrichment_build_ledger_unexplained_remainder() {
    let cert = eval(ArtifactKind::CacheEntry, "unexp", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 800_000);
...
fn enrichment_build_ledger_serde_roundtrip() {
    let cert = eval(ArtifactKind::CodeLayout, "led-serde", &cell_arm_nv2(), &cell_arm_a78(), 1_000_000, 900_000);
    let components = vec![ResidualComponent::new("layout", 500_000, 450_000, "reorder cost")];
    let ledger = build_residual_ledger(&cert, components).unwrap();
...
fn enrichment_build_ledger_deterministic_hash() {
    let cert = eval(ArtifactKind::CacheEntry, "det-led", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 800_000);
...
fn enrichment_build_ledger_display_contains_key_info() {
    let cert = eval(ArtifactKind::RewriteRule, "disp-led", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 800_000);
    let ledger = build_residual_ledger(&cert, vec![]).unwrap();
...
fn enrichment_build_ledger_component_by_name_found() {
    let cert = eval(ArtifactKind::CacheEntry, "lookup", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 800_000);
    let components = vec![
...
fn enrichment_build_ledger_survival_fraction() {
    let cert = eval(ArtifactKind::CacheEntry, "surv-led", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 800_000);
    let components = vec![
...
fn enrichment_build_ledger_survival_fraction_zero_source() {
    let cert = eval(ArtifactKind::RewriteRule, "zero-surv", &cell_x86_zen4(), &cell_x86_zen4(), 1_000_000, 1_000_000);
    let ledger = build_residual_ledger(&cert, vec![]).unwrap();
...
fn enrichment_manifest_covers_all_outcome_types() {
    ...
    assert!(outcomes.len() >= 3, "manifest should cover at least 3 outcome types");
}
...
fn enrichment_manifest_covers_diverse_artifact_kinds() {
    ...
    assert!(kinds.len() >= 5, "manifest should cover at least 5 artifact kinds, got {}", kinds.len());
}
...
fn enrichment_manifest_has_cross_arch_certificates() {
    ...
    assert!(cross_arch_count >= 2, "manifest should have at least 2 cross-arch certificates");
}
...
fn enrichment_manifest_has_same_arch_certificates() {
    ...
    assert!(same_arch_count >= 2, "manifest should have at least 2 same-arch certificates");
}
...
fn enrichment_manifest_certificate_ids_unique() {
    ...
    assert_eq!(ids.len(), certs.len(), "all certificate IDs should be unique");
}
...
fn enrichment_manifest_all_cert_ids_start_with_tc() {
    let certs = franken_engine_transport_manifest();
    for cert in &certs {
        assert!(cert.certificate_id.starts_with("tc-"), "cert ID {} should start with tc-", cert.certificate_id);
    }
}
...
fn enrichment_manifest_summary_single_full_transport() {
    let cell = cell_x86_zen4();
    let cert = eval(ArtifactKind::RewriteRule, "single-full", &cell, &cell, 1_000_000, 1_000_000);
...
fn enrichment_manifest_summary_single_failed() {
    let cert = eval(ArtifactKind::RewriteRule, "single-fail", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 100_000);
    let summary = TransportManifestSummary::build(&[cert]);
...
fn enrichment_manifest_summary_usability_rate_mixed() {
    let cell = cell_x86_zen4();
    let c1 = eval(ArtifactKind::RewriteRule, "usable-1", &cell, &cell, 1_000_000, 1_000_000);
    let c2 = eval(ArtifactKind::CacheEntry, "usable-2", &cell, &cell_x86_alder(), 1_000_000, 800_000);
    let c3 = eval(ArtifactKind::ProfileData, "failed-3", &cell, &cell_x86_alder(), 1_000_000, 100_000);
...
fn enrichment_transport_event_from_certificate_fields_match() {
    let cert = eval(ArtifactKind::CacheEntry, "evt-match", &cell_x86_zen4(), &cell_arm_nv2(), 1_000_000, 500_000);
    let evt = TransportEvent::from_certificate(&cert, TransportEventKind::CertificateCreated, epoch(42));
    ...
    assert_eq!(evt.residual_fraction_millionths, cert.residual_fraction_millionths);
    assert_eq!(evt.epoch, epoch(42));
}

#[test]
fn enrichment_transport_event_different_kinds_same_cert() {
    let cert = eval(ArtifactKind::RewriteRule, "evt-kinds", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 900_000);
    let e1 = TransportEvent::from_certificate(&cert, TransportEventKind::CertificateCreated, epoch(1));
    let e2 = TransportEvent::from_certificate(&cert, TransportEventKind::LedgerBuilt, epoch(1));
    let e3 = TransportEvent::from_certificate(&cert, TransportEventKind::CertificateInvalidated, epoch(1));
    let e4 = TransportEvent::from_certificate(&cert, TransportEventKind::TransportReEvaluated, epoch(1));
    // Same cert but different event kinds -> different content hashes
    let hashes: BTreeSet<_> = [e1.content_hash, e2.content_hash, e3.content_hash, e4.content_hash]
        .iter()
        .copied()
        .collect();
    assert_eq!(hashes.len(), 4, "different event kinds should produce different content hashes");
}

#[test]
fn enrichment_transport_event_different_epochs_different_hashes() {
    let cert = eval(ArtifactKind::ProfileData, "evt-epoch", &cell_arm_nv2(), &cell_arm_a78(), 1_000_000, 950_000);
    let e1 = TransportEvent::from_certificate(&cert, TransportEventKind::CertificateCreated, epoch(1));
    let e2 = TransportEvent::from_certificate(&cert, TransportEventKind::CertificateCreated, epoch(2));
    assert_ne!(e1.content_hash, e2.content_hash);
}

#[test]
fn enrichment_transport_event_serde_roundtrip() {
    let cert = eval(ArtifactKind::SpeculationGuard, "evt-serde", &cell_x86_avx512(), &cell_x86_zen4(), 1_000_000, 700_000);
...
fn enrichment_transport_event_display_contains_key_info() {
    let cert = eval(ArtifactKind::AotModule, "evt-disp", &cell_x86_zen4(), &cell_arm_nv2(), 1_000_000, 200_000);
    let evt = TransportEvent::from_certificate(&cert, TransportEventKind::CertificateCreated, epoch(10));
    let s = evt.to_string();
...
fn enrichment_full_pipeline_evaluate_build_ledger_validate_event() {
    ...
    let cert = eval(ArtifactKind::SynthesizedKernel, "pipeline", &src, &tgt, 1_000_000, 700_000);
    ...
    // Create event
    let evt = TransportEvent::from_certificate(&cert, TransportEventKind::CertificateCreated, epoch(100));
    assert_eq!(evt.outcome, cert.outcome);
...
fn enrichment_manifest_to_summary_to_event_pipeline() {
    ...
    let events: Vec<_> = certs
        .iter()
        .map(|c| TransportEvent::from_certificate(c, TransportEventKind::CertificateCreated, epoch(50)))
        .collect();
...
fn enrichment_cross_arch_roundtrip_both_directions() {
    ...
    let x86 = cell_x86_zen4();
    let arm = cell_arm_nv2();
    let c1 = eval(ArtifactKind::ProfileData, "x2a", &x86, &arm, 1_000_000, 600_000);
    let c2 = eval(ArtifactKind::ProfileData, "a2x", &arm, &x86, 1_000_000, 600_000);
...
fn enrichment_arch_sensitive_identity_transport() {
    ...
    for kind in ArtifactKind::ALL {
        if kind.is_arch_sensitive() {
            let cert = eval(*kind, &format!("id-{}", kind.as_str()), &cell, &cell, 1_000_000, 1_000_000);
            assert_eq!(cert.outcome, TransportOutcome::FullTransport);
        }
    }
}
...
fn enrichment_residual_ledger_balance_check_on_build() {
    ...
    let cert = eval(ArtifactKind::CacheEntry, "balance", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 800_000);
    let components = vec![
...
fn enrichment_performance_loss_zero_for_full_transport() {
    let cell = cell_arm_nv2();
    let cert = eval(ArtifactKind::RewriteRule, "zero-loss", &cell, &cell, 1_000_000, 1_000_000);
    assert_eq!(cert.performance_loss_millionths(), 0);
}
...
fn enrichment_performance_loss_saturates_at_million() {
    ...
    let cert = eval(ArtifactKind::RewriteRule, "max-loss", &cell_x86_zen4(), &cell_x86_alder(), 1_000_000, 0);
